...
use windows::{
    Win32::{
        ...
        UI::WindowsAndMessaging::{
            EnumWindows, GCLP_HICONSM, GetClassLongPtrW, GetForegroundWindow, GetIconInfo,
            GetWindowDisplayAffinity, GetWindowTextW, GetWindowThreadProcessId, HICON,
            ICON_SMALL2, ICONINFO, IsWindowVisible, SendMessageW,
            SetWindowDisplayAffinity, WDA_EXCLUDEFROMCAPTURE, WDA_NONE, WM_GETICON,
        },
    },
    core::BOOL,
};
...
/// Returns the title of a window by HWND handle.
pub fn get_window_title(hwnd: u32) -> String {
    let hwnd_val = HWND(hwnd as *mut _);
    let mut buf = [0u16; 128];
    let len = unsafe { GetWindowTextW(hwnd_val, &mut buf) };
    if len == 0 {
...
/// Returns (hwnd, pid) of the current foreground window, or (0, 0) on failure.
pub fn get_foreground_window() -> (u32, u32) {
    unsafe {
...
/// Hide or show our own window in screen capture.
///
/// Called directly on our own HWND — no DLL injection required.
/// SetWindowDisplayAffinity can be called by the owning process directly.
/// `exclude = true`  → window shows as black in any screen share / recording.
/// `exclude = false` → window is visible in screen share again.
pub fn set_self_capture_visibility(hwnd: u32, exclude: bool) {
